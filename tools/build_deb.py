...
def main() -> int:
    ...
    if os.path.isfile(args.srt):
        ...
        import make_timeline  # noqa: E402
        items = make_timeline.build(make_timeline.read_srt(args.srt), args.offset)
        buf = io.StringIO()
        buf.write("# miku-voicebank lyric timeline - generated by tools/make_timeline.py\n")
        buf.write("# source: %s  (song starts at %s in the source video)\n"
                  % (os.path.basename(args.srt), make_timeline.SONG_START))
        ...
        for t, kind, text in items:
            buf.write("%.3f\t%s\t%s\n" % (t, kind, text))
        timeline = buf.getvalue().encode("utf-8")
